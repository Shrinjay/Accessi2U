...
export class GeolocationService extends EventTarget {
  ...
  async requestPermissions(): Promise<void> {
    ...
    this.geolocationPermissionState = await new Promise<PermissionState>((resolve, reject) => {
  ...
  private handleDeviceOrientation = (ev: DeviceOrientationEvent) => {
    ...
    if (!ev.absolute && ev.webkitCompassHeading) {
      ...
      this.compassHeading = eulerAnglesToCompassHeading(ev.alpha, ev.beta, ev.gamma);
    }
